...
public class ClientResource {
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the clientDTO, or with status 404 (Not Found)
     */
    @GetMapping("/filter/clients/{id}")
    @Timed
    public ResponseEntity<List<Client>> getEntrepriseClient(@PathVariable Long id) {
        log.debug("REST request to get Client : {}", id);
        List<Client> clients = clientService.findEntrepriseClient(id);
